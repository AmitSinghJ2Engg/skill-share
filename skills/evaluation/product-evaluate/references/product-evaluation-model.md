...
## Bigin Enforcement Implementation

For each gate, a **Bigin workflow rule** fires on stage transition:

```
Rule pattern:
  Trigger: Record enters Stage [X]
  Condition: [check required fields]
  Action on FAIL: Revert stage to previous + create Activity "Gate [N] blocked: [reason]"
  Action on PASS: (allow transition — no action needed)
```

**Implementation path:**
1. `zoho-solutions-architect` produces Implementation Notes for: stage configuration + 3 new fields + workflow rules
2. `zoho-developer` writes any Deluge custom functions for complex checks (e.g., Gate 4 cross-module Vendor_Grade check)
3. Bigin admin configures stages + workflow rules per implementation notes

**Limitation:** Bigin workflow rules can check field values but cannot do cross-module lookups natively (e.g., checking Vendor_Grade on a linked Contact). For Gate 4, the check either:
(a) Uses a Deluge function triggered by workflow to do the cross-lookup, or
(b) Copies Vendor_Grade to a Pipeline field via Flow when Supplier is linked (denormalization)

Recommend option (b) — simpler, no code, keeps enforcement in Bigin config layer.
...
```
Artifact (calculates)          →    Bigin (stores verdict)     →    Bigin (enforces)
───────────────────────────────     ──────────────────────     ──────────────────────
Opportunity assessment (4 cr)  →    Opportunity_Score (int)    →    G2: ≥55 to proceed
Unit economics calculator      →    Financial_Viability (pck)  →    G3: Pass/Marginal
Scale decision analyzer        →    Scale_Verdict (picklist)   →    G7: Scale/Pivot
Vendor evaluator (3 tiers)     →    Vendor_Score + Grade       →    G4: Grade A/B/C
```

Artifacts compute the intelligence. Bigin stores the verdict. Bigin enforces the gate.
The artifact is the source of truth for HOW the verdict was reached (detailed scoring, data, rationale).
Bigin is the source of truth for WHERE the product is in the lifecycle (stage + gate fields).
