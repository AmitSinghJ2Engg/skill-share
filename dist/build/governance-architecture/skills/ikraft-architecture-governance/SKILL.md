...
4. Read `tools/plugin-registry.json` — load current plugin/skill registry
...
26. Context objects must be centralized — never embedded inline (LAW-6)
...
systems_accessed:
  - plugin registry (read — tools/plugin-registry.json)
  - context files (read — context/system-ops/, context/product-pipeline/)
write_permissions: []
