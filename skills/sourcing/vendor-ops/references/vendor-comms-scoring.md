...
## Extended Question Bank (from Vendor_Evaluation_Tracker_1.xlsx)

The Tracker spreadsheet adds these enrichments to the same questions:

| Column | Content |
|---|---|
| Scoring Rule | Named rules: "Factory Capability Rule", "Documentation & Credibility Rule" — group related questions |
| Applies to | Which vendor type: "Dropshipping Partner", "Factory", or both |
| Quality Check | Whether the aspect requires a QC step |
| Audit Required | Whether the aspect needs periodic re-audit |
| Catalogue Shared | Bonus question (not in comms sheet): "Catalog or portfolio provided?" — 3 if yes, 0 if no |
