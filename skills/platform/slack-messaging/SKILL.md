...
description: >
  SM- Formats and sends Slack messages using correct mrkdwn syntax. Use this skill
  whenever composing, drafting, or posting ANY Slack message — including automated
  task summaries, gate decision alerts, pipeline notifications, artifact "Send to
  Slack" payloads, and manual messages. Trigger for: "send to slack", "post to
  slack", "slack message", "notify slack", "slack alert", "share to slack",
  slack_send_message, slack_send_message_draft, slack_create_canvas, "#ism-".
  All Slack output in the Ismokraft system MUST route through this skill.
---

# Slack Messaging Best Practices

This skill provides guidance for composing well-formatted, effective Slack messages. It is the single gateway for all Slack output in the Ismokraft system.

## When to Use

Apply this skill whenever composing, drafting, or helping the user write a Slack message — including when using `slack_send_message`, `slack_send_message_draft`, or `slack_create_canvas`. This includes automated task output (daily discovery summaries, gate alerts, kill notifications) and user-triggered "Send to Slack" actions from artifacts.
