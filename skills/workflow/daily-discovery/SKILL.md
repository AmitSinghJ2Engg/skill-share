...
description: >
  Daily product discovery pipeline — determine today's zone rotation, generate
  keywords, crawl marketplaces in batch, score candidates, persist to CRM,
  capture learning signals, post summary to Slack. Runs headless via scheduled
  task or interactively via /daily-discovery.
  ALWAYS trigger for: "run daily discovery", "product discovery pipeline",
  "zone rotation discovery", "keyword generation + crawl".
...
metadata:
  ...
  skills_invoked:
    ...
    - zoho-data-ops:WRITE
    - product-screen:SCORE
    - product-screen:REPORT
...
**Partial failure:** If some marketplaces are unreachable, the skill returns candidates from available marketplaces with null signals for unreachable ones. The workflow continues with available data.
...
- This workflow is an **orchestrator**. It invokes skills by mode. It does NOT implement crawling, scoring, or filtering logic.
...
- This workflow only CREATES new Product_Launches records (via ZO skill). It does not modify existing records (except adding scores to records it created in the same run, also via ZO skill).
- Business skills (PD, PS) produce data only. All CRM/Bigin writes go through `zoho-data-ops` (ZO).
- If this workflow has already run today (dedup check), do not run again.
