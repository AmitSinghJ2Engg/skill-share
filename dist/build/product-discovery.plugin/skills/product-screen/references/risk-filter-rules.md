...
Check across all marketplaces where the product was found. A trademark issue on any marketplace is a global concern.
...
Evidence sources: Google Trends seasonal pattern (India + US geos), Amazon BSR history (India or US, if available), Etsy seasonal listing patterns, category knowledge from opportunity map (in project knowledge).

Use multi-marketplace trend data for more reliable seasonality assessment than any single platform alone.

## Filter 3 — Certification

Criteria: Product requires mandatory certifications to sell on target marketplaces.
...
**US (Amazon.com):**
PASS: Most wooden home decor products. No CPSC, FDA, or FCC requirements.
CONDITIONAL: Products for children under 12 may need CPSIA testing. Food-contact items need FDA compliance.
FAIL: Mandatory CPSC testing for children's products, FDA for food-contact without compliance path.

**Europe / Australia:** Note requirements for future expansion. Not blocking for initial launch but flag for awareness.

Note: Most wooden home decor and gifting products are PASS across India and US. Flag kitchen items that contact food on any marketplace.
...
Note: Wood products generally score well here. Watch for combined materials (wood + glass, wood + ceramic). Applies equally across all FBA warehouses (India, US, etc.).
...
6. Note which marketplace(s) triggered each filter result.
